...
let
  ...
  fixUpContext = v:
    ...
    in if __getContext v == {} && storeDirMatch != null
      then __appendContext v { ${__head storeDirMatch} = { path = true; }; }
      else v;

  replaceWithInputMap = v:
    let
      path =
        lib.findFirst builtins.pathExists null
          (lib.mapAttrsToList
            (repo-url: repo-path: repo-path + lib.strings.removePrefix repo-url v.url)
            (lib.filterAttrs (repo-url: repo-path: lib.hasPrefix repo-url v.url)
              parentConfig.inputMap));
    in
    # There an assumption here about what kind of derivation src is
    # we assume it's fetchurl since this is hardcoded in nix-tools
    if v ? "url" && v ? "outputHash" && v.outputHashAlgo == "sha256" && path != null
    then builtins.path {
      inherit path;
      sha256 = v.outputHash;
      name = "${config.name}.tar.gz";
      recursive = false;
    }
    else v;
...
in {
  ...
  config._module.args = mod_args // { inherit (config) flags; };
