...

{
  ...
  config.nonReinstallablePkgs =
    ...
    ++ lib.optionals (
      __elem config.compiler.nix-name ["ghc901" "ghc902" "ghc921" "ghc922" "ghc923" "ghc924" "ghc925" "ghc941" "ghc942"]) [
      "ghc-bignum" ]
    ++ lib.optionals (
      __elem config.compiler.nix-name ["ghc925" "ghc941" "ghc942"]) [
      "system-cxx-std-lib" ]
  ...
  config.bootPkgs =  [
    ...
    ++ lib.optionals (
      ...
      "ghc-bignum" ]
    ++ lib.optionals (
      __elem config.compiler.nix-name ["ghc941" "ghc942"]) [
      "system-cxx-std-lib" ];
