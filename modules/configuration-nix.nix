...
in addPackageKeys {
  ...
  packages.ghc.components.library.build-tools = lib.mkIf
    (config.ghc.package ? configured-src)
    (lib.mkForce (
  ...
  packages.ghc.package-description-override = lib.mkIf
    (config.ghc.package ? configured-src)
    (pkgs.lib.mkForce null);
