...
let
  ...
  hadrianArgs = "--flavour=${
      ...
      #
      # For now we only do this for native compilers. Cross compilers do not seem to know
      # -mno-outline-atomics, they do know -fno-inline-atomics though. Hooray, everything
      # is inconsistent.
      + lib.optionalString (!haskell-nix.haskellLib.isCrossTarget && targetPlatform.isLinux && targetPlatform.isAarch64) " '*.*.ghc.c.opts += -optc-mno-outline-atomics'"
