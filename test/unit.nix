...
lib.runTests {
  ...
  testParseRepositoryBlock = {
      expr = __toJSON (haskellLib.parseRepositoryBlock pkgs.buildPackages "cabal.project" {} {}
        pkgs.buildPackages.haskell-nix.cabal-install.${compiler-nix-name}
        pkgs.buildPackages.haskell-nix.nix-tools.${compiler-nix-name} ''
          ghcjs-overlay
