...
in builtins.listToAttrs (builtins.concatMap (system: builtins.concatMap (compiler-nix-name:
  ...
  { name = "${prefix}-extra";         value = (pkgs.roots' compiler-nix-name).ghc-extra-projects-nix or {}; }
  ...
  { name = "${prefix}-windows";       value = (pkgsForWindows.pkgsCross.mingwW64.roots' compiler-nix-name).ghc-extra-projects-nix or {}; }
  ...
  { name = "${prefix}-musl";          value = (pkgs.pkgsCross.musl64.roots' compiler-nix-name).ghc-extra-projects-nix or {}; }
  ...
  { name = "${prefix}-arm";           value = (pkgs.pkgsCross.aarch64-multiplatform.roots' compiler-nix-name).ghc-extra-projects-nix or {}; }
  ...
  { name = "${prefix}-ghcjs";         value = (pkgs.pkgsCross.ghcjs.roots' compiler-nix-name).ghc-extra-projects-nix or {}; }
