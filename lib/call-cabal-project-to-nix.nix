...
, src, materialized-dir ? ../materialized
...
, plan-sha256 ?
  null # The hash of the plan-to-nix output (makes the plan-to-nix step a fixed output derivation)
, materialized ? null # Location of a materialized copy of the nix files
, checkMaterialization ?
  null # If true the nix files will be generated used to check plan-sha256 and material
, cabalProjectFileName ? "cabal.project", cabalProject ? null
, cabalProjectLocal ? null, cabalProjectFreeze ? null, caller ?
  "callCabalProjectToNix" # Name of the calling function for better warning messages
, compilerSelection ? p: p.haskell-nix.compiler
, ghc ? null # Deprecated in favour of `compiler-nix-name`
, ghcOverride ?
  null # Used when we need to set ghc explicitly during bootstrapping
...
, extra-hackage-tarballs ? { }, source-repo-override ?
  { } # Cabal seems to behave incoherently when
...
let
  ...
  nix-tools = if args.nix-tools or null != null then
    args.nix-tools
  else
  ...
  ghc' = if ghcOverride != null then
    ghcOverride
  else if ghc != null then
    __trace ("WARNING: A `ghc` argument was passed" + forName
...
in let
  ghc = if ghc' ? latestVersion then
    __trace
    "WARNING: ${ghc'.version} is out of date, consider using upgrading to ${ghc'.latestVersion}."
    ghc'
  else
    ghc';
  ...
  cleanedSource = haskellLib.cleanSourceWith {
    name = if name != null then
      "${name}-root-cabal-files"
    else
      "source-root-cabal-files";
    src = src.origSrc or src;
    filter = path: type:
      (!(src ? filter) || src.filter path type) && (type == "directory"
  ...
  index-state-max = if index-state != null then
    index-state
  else
  ...
  cached-index-state = if index-sha256 != null then
    index-state-max
  else
    let
      suitable-index-states = builtins.filter
        (s: s >= index-state-max) # This compare is why we need zulu time
        (builtins.attrNames index-state-hashes);
    in if builtins.length suitable-index-states == 0 then
      index-state-max
    else
      pkgs.lib.head suitable-index-states;

  # Lookup hash for the index state we found
  index-sha256-found = if index-sha256 != null then
    index-sha256
  else
    index-state-hashes.${cached-index-state} or null;

in assert (if index-sha256-found == null then
  throw
  "Unknown index-state ${index-state-max}, the latest index-state I know about is ${
    pkgs.lib.last (builtins.attrNames index-state-hashes)
...
let
  ...
  replaceSourceRepos = projectFile:
    let
      fetchPackageRepo = fetchgit: repoData:
        let
          fetched =
            if inputMap ? "${repoData.url}/${repoData.rev or repoData.ref}" then
              inputMap."${repoData.url}/${repoData.rev or repoData.ref}"
            else if inputMap ? ${repoData.url} then
              (if inputMap.${repoData.url}.rev
              != (repoData.rev or repoData.ref) then
                throw ''
                  ${inputMap.${repoData.url}.rev} may not match ${
                    repoData.rev or repoData.ref
                  } for ${repoData.url} use "${repoData.url}/${
                    repoData.rev or repoData.ref
                  }" as the inputMap key if ${
                    repoData.rev or repoData.ref
                  } is a branch or tag that points to ${
                    inputMap.${repoData.url}.rev
                  }.''
              else
                inputMap.${repoData.url})
            else if repoData.sha256 != null then
              fetchgit {
              ...
              let
                drv = builtins.fetchGit {
                  ...
                  ref = repoData.ref or null;
                }
                # fetchGit does not accept "null" as rev, so when it's null
                # we have to omit the argument completely.
                  // pkgs.lib.optionalAttrs (repoData ? rev) {
                    inherit (repoData) rev;
                  };
              in __trace
              "WARNING: No sha256 found for source-repository-package ${repoData.url} ref=${
                repoData.ref or "(unspecified)"
              } rev=${
                repoData.rev or "(unspecified)"
              } download may fail in restricted mode (hydra)" (__trace
                "Consider adding `--sha256: ${
                  hashPath drv
                }` to the ${cabalProjectFileName} file or passing in a sha256map argument"
                drv);
        in {
      ...
      sourceRepoPackageResult =
        pkgs.haskell-nix.haskellLib.parseSourceRepositoryPackages
        cabalProjectFileName sha256map source-repo-override projectFile;

      sourceRepoFixedProjectFile = sourceRepoPackageResult.initialText
        + pkgs.lib.strings.concatMapStrings (block:
          if block ? sourceRepo then
            let f = fetchPackageRepo evalPackages.fetchgit block.sourceRepo;
            in ''

              ${block.indentation}source-repository-package
              ${block.indentation}  type: git
              ${block.indentation}  location: file://${f.location}
              ${block.indentation}  subdir: ${
                builtins.concatStringsSep " " f.subdirs
              }
              ${block.indentation}  tag: ${f.tag}
            '' + block.followingText
          else
            block.followingText) sourceRepoPackageResult.sourceRepos;
      ...
      sourceReposEval =
        builtins.map (x: (fetchPackageRepo evalPackages.fetchgit x.sourceRepo))
        sourceRepoPackageResult.sourceRepos;
      sourceReposBuild =
        builtins.map (x: (fetchPackageRepo pkgs.fetchgit x.sourceRepo).fetched)
        sourceRepoPackageResult.sourceRepos;

      # Parse the `repository` blocks
      repoResult = pkgs.haskell-nix.haskellLib.parseRepositories evalPackages
        cabalProjectFileName sha256map inputMap nix-tools
    ...
    in {
      ...
      makeFixedProjectFile = ''
        HOME=$(mktemp -d)
        cp -f ${
          evalPackages.writeText "cabal.project" sourceRepoFixedProjectFile
        } ./cabal.project
        chmod +w -R ./cabal.project
      '' + pkgs.lib.strings.concatStrings (map (f: ''
      ...
      replaceLocations = pkgs.lib.strings.concatStrings
        (pkgs.lib.lists.zipListsWith (n: f: ''
          (cd $out${subDir'}
          substituteInPlace $tmp${subDir'}/dist-newstyle/cache/plan.json --replace file://${f.location} ${
            builtins.toString n
          }
          for a in $(grep -rl file://${f.location} .plan.nix/*.nix); do
            substituteInPlace $a --replace file://${f.location} ${
              builtins.toString n
            }
          done)
  ...
  platformString = p:
    with p.parsed;
    "${cpu.name}-${vendor.name}-${kernel.name}";
  ...
  dummy-ghc = evalPackages.writeTextFile {
    ...
    text = ''
      ...
      ''}
        --supported-languages*)
          cat ${
            import ./supported-languages.nix { inherit pkgs evalPackages ghc; }
          }
          ;;
        ...
        --info*)
          echo '[("target os", "${
            if pkgs.stdenv.targetPlatform.isLinux then
              "OSLinux"
            else if pkgs.stdenv.targetPlatform.isDarwin then
              "OSDarwin"
            else if pkgs.stdenv.targetPlatform.isWindows then
              "OSMinGW32"
            else if pkgs.stdenv.targetPlatform.isGhcjs then
              "OSGhcjs"
            else
              throw "Unknown target os ${pkgs.stdenv.targetPlatform.config}"
          }")'
          echo ',("target arch","${
            if pkgs.stdenv.targetPlatform.isx86_64 then
              "ArchX86_64"
            else if pkgs.stdenv.targetPlatform.isAarch64 then
              "ArchAArch64"
            else if pkgs.stdenv.targetPlatform.isJavaScript then
              "ArchJavaScript"
            else
              throw "Unknown target arch ${pkgs.stdenv.targetPlatform.config}"
          }")'
          echo ',("target platform string","${
            platformString pkgs.stdenv.targetPlatform
          }")'
          echo ',("Build platform","${
            platformString pkgs.stdenv.buildPlatform
          }")'
  ...
  plan-json = materialize ({
    ...
    LOCALE_ARCHIVE = pkgs.lib.optionalString
      (evalPackages.stdenv.buildPlatform.libc == "glibc")
      "${evalPackages.glibcLocales}/lib/locale/locale-archive";
    LANG = "en_US.UTF-8";
    ...
    # packages used by the solver (cached-index-state >= index-state-max).
      pkgs.lib.optionalString (index-state != null)
    ...
    (cd $out${subDir'} && plan-to-nix --full ${
      if ignorePackageYaml then "--ignore-package-yaml" else ""
    } --plan-json $tmp${subDir'}/dist-newstyle/cache/plan.json -o .)
