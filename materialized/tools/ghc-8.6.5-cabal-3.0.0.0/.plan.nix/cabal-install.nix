    ...
    components = {
      exes = {
        "cabal" = {
          ...
          mainPath = ((((((([
            "Main.hs"
            ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "8.0") "") ++ (pkgs.lib).optional (system.isAix) "") ++ (pkgs.lib).optional (!(compiler.isGhc && (compiler.version).ge "8.0")) "") ++ (pkgs.lib).optionals (flags.native-dns) ([
            ""
            ] ++ [ "" ])) ++ [
            ""
