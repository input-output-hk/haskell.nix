...
, checkMaterialization ? false }:
 let
  ...
  crossSystems = nixpkgsName: nixpkgs: compiler-nix-name: system:
    ...
    let lib = nixpkgs.lib;
    in lib.optionalAttrs (nixpkgsName == "R2003" && compiler-nix-name != "ghc8101" && compiler-nix-name != "ghc8102") {
    inherit (lib.systems.examples) ghcjs;
  } // lib.optionalAttrs (system == "x86_64-linux" && compiler-nix-name != "ghc8101" && compiler-nix-name != "ghc8102") {
