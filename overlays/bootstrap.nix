    ...
    compiler =
        let bootPkgs = with final.buildPackages; {
            ...
            ghc-patches = version: let
                ...
                ++ final.lib.optional (versionAtLeast "8.10"                            && final.stdenv.targetPlatform.isAarch64) ./patches/better-symbol-addr-debug.patch
                ++ final.lib.optional (versionAtLeast "8.10"                            && final.stdenv.targetPlatform.isAarch64) ./patches/aarch64-handle-none-rela.patch
                ;
        in ({
